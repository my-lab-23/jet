    ...
    Column(
        ...
        for(msg in msgs.value) {
            ...
            if(c==0 && msg!="") {
                ...
                Text(msg, fontSize = 25.sp, modifier = Modifier.background(Color.LightGray)
                    .fillMaxWidth())
                c = 1
            } else if((c==1 && msg!="")) {

                Text(msg, fontSize = 25.sp, modifier = Modifier.background(Color.Gray)
                    .fillMaxWidth())
                c = 0
            }
        }
...
fun msgs(): List<String> {
    ...
    return FileReader("/home/ema/data/data").readLines().toString()
        .replace("[", "").replace("]", "").split("|*|").reversed()
}
